...
The following table summarises the `fibers-server` throughput under load generated by [wrk2](https://github.com/giltene/wrk2). The server was allocated 4 cores and 3GB memory limits.
...
Request handling consists of 80% sleep, 15% IO and 5% CPU. The raw results are [here](./benchmark/experiment-1).
...
Request handling consists of 95% sleep, 4.5% IO and 0.5% CPU. The raw results are [here](./benchmark/experiment-2).
...
Request handling consists of 95% sleep, 4.5% IO and 0.5% CPU. The raw results are [here](./benchmark/experiment-3).


The server and load generator are deployed as two containers inside a single Kubernetes Pod communicating via localhost on the shared network interface.

The server and load generator are deployed in two different Kubernetes Pods communicating using Kubernetes [cluster routing](https://kubernetes.io/docs/concepts/cluster-administration/networking).
